...
if __name__ == "__main__":
    ...
    for n_A in range(0,N_wells_A):
        ...
        for n_B in range(0,N_wells_B):
            ...
            H_list_B[:,n_B] = kron(I_A,LCJ_obj_B.H_wl[n_A])
    ...
    num_outpoint_points = 2**output_resolution_order
    time_increment_gate = T_gate/num_outpoint_points
    time_increment_phi2 = t_phi2/num_outpoint_points
    ...
    exp_logical_state = array([cos(theta_A/2)*cos(theta_B/2),cos(theta_A/2)*sin(theta_B/2)*exp(1j*phi_B),
                            sin(theta/2)*cos(theta_B/2)*exp(1j*phi_A),sin(theta_A/2)*sin(theta_B/2)*exp(1j*(phi_A+phi_B+target_phase))] )
    ...
    spins_A   = zeros((N_samples,2*num_outpoint_points,3))
    spins_B   = zeros((N_samples,2*num_outpoint_points,3))
    spins_AB  = zeros((N_samples,2*num_outpoint_points,3))

    # Fidelities with expected logical state
    fidelities = zeros((N_samples,2*num_outpoint_points))
    ...
    for sample_num in range(0,N_samples):
        ...
        for output_index in range(num_outpoint_points):
        ...
        for output_index2 in range(num_outpoint_points):
    ...
    with open(save_file,'wb') as f:
        ...
        for i in range(N_samples):
            ...
            for k in range(2*num_outpoint_points):
